...
async function getQuote(postalCode) {
  ...
  if (!token) throw new Error("SUPERFRETE_TOKEN não configurado no Netlify.");
  ...
  const response = await fetch(`${base}/api/v0/calculator`, {
    ...
    headers: {
      ...
      "User-Agent": "JLMAKEUP/1.0 (JLMAKEUP26@GMAIL.COM)",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      data?.message ||
      data?.error ||
      `A SuperFrete recusou a cotação (HTTP ${response.status}).`
    );
  }
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (postalCode.length !== 8) {
      return json(400, { error: "CEP de destino inválido." });
    }
    ...
    console.error(error);
    return json(502, { error: error.message || "Erro ao calcular o frete." });
  }
};
